...
public class gjBodyFactory {
    ...
    private gjBodyFactory(World world){
        this.world = world;
    }

    public static gjBodyFactory getInstance(World world){
        if(thisInstance == null){
            thisInstance = new gjBodyFactory(world);
        }
        return thisInstance;
    }
    ...
    public Body addCircleFixture(Body bod, float x, float y, float size, int material){
        CircleShape circleShape = new CircleShape();
        circleShape.setRadius(size /2);
        circleShape.setPosition(new Vector2(x,y));
        bod.createFixture(makeFixture(material,circleShape));
        circleShape.dispose();
        return bod;
    }
